 ...
 * 
 * A search query is only executed against the default index.
 */
public class MESQuery {
    private static final Log log = LogFactory.getLog(MESQuery.class);

    private final MultiElasticSearchContext parent;

    public MESQuery(MultiElasticSearchContext parent) {
        this.parent = parent;
    }
    ...
    private String doTheQuery(String queryString) {
        ...
        try {
            String url = parent.getDefaultIndexUrl() + "/_search";
